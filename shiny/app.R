...
# ── Posit Cloud path safety ───────────────────────────────────
# here() anchors to the .Rproj file in the project root.
# If here() can't find .Rproj, it falls back to the working directory.
# On Posit Cloud: always open this as a Project (not a plain folder)
# so that .Rproj is found correctly.

# ── Safe data loading with clear error messages ───────────────
safe_load <- function(path, label) {
  ...
  if (!file.exists(full_path)) {
    stop(paste0(
      ...
      "  This file is generated by running the analysis scripts.\n",
      "  In RStudio (or Posit Cloud terminal), run:\n\n",
...
bangladesh <- safe_load("data/processed/bangladesh_amr.rds",  "Bangladesh phenotypic data")
bd_genomic <- safe_load("data/processed/bd_genomic.rds",      "Bangladesh WGS genomic data")
sa_genomic <- safe_load("data/processed/sa_genomic.rds",      "South Asia genomic data")
